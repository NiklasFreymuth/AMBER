...
class AbstractMetaModule(torch.nn.Module, abc.ABC):
    """
    An abstract class for the modules used in the GNN. They are used for updating node-, edge features.
    """
    ...
    def _multiscatter(self, features: torch.Tensor, indices: torch.Tensor, dim: int, dim_size: int):
        """
        This function is used to aggregate over the nodes/edges. Uses an internal list of scatter recude
