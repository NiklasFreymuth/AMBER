...
from src.hmpn.abstract.abstract_graph_assertions import AbstractGraphAssertions
from src.hmpn.abstract.abstract_input_embedding import AbstractInputEmbedding
from src.hmpn.abstract.abstract_stack import AbstractStack
from src.hmpn.common.hmpn_util import get_scatter_reducers, get_create_copy
...
class AbstractMessagePassingBase(nn.Module, abc.ABC):
    ...
    def __init__(self, *,
        ...
        if edge_dropout > 0.0:
            self.maybe_edge_dropout = self._edge_dropout
            self.dropout_prob = edge_dropout
        else:
    ...
    def _edge_dropout(self, graph: Batch):
        from torch_geometric.utils import dropout_edge
        graph.edge_index, edge_mask = dropout_edge(edge_index=graph.edge_index,
                                                   p=self.dropout_prob,
    ...
    def unpack_features(self, graph: Batch):
        """
    ...
    def forward(self, graph: Batch) -> Batch:
        ...
        Returns:
            Either a modified graph or a tuple of (node_features, edge_features), depending on the
                ...
                All node, edge and went through an embedding and multiple rounds of
                message passing.
        ...
        self._process_encoded_graph(graph)
        return self.maybe_transform_output(graph)

    def __repr__(self):
        return super().__repr__()[:-3] + f"\n edge_dropout: {self.dropout_prob}" + "\n)"
