...
from src.recording import create_loggers
...
class Recorder:
    ...
    def __init__(self, config: ConfigDict, algorithm: AbstractIterativeAlgorithm):
        ...
        Args:
            ...
            algorithm:
        """
        self._loggers: List[AbstractLogger] = create_loggers(config=config, algorithm=algorithm)
        self._algorithm = algorithm
        self._writer = get_logging_writer(
    ...
    def record_iteration(self, iteration: int, recorded_values: ValueDict) -> ValueDict:
        ...
        for logger in self._loggers:
            #try:
                logger.log_iteration(recorded_values=recorded_values, iteration=iteration)
            # except Exception as e:
            #     self._writer.error(
            #         "Error with logger '{}': {}".format(
            #             logger.__class__.__name__, repr(e)
            #         )
            #     )
