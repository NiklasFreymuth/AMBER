...
class CustomWAndBLogger(AbstractLogger):
    ...
    def __init__(self, config: ConfigDict, algorithm: AbstractIterativeAlgorithm):
        ...
        self.plot_first_iteration = wandb_params.get("plot_first_iteration", False)
        ...
        environment_name = get_from_nested_dict(
            config, list_of_keys=["environment", "environment"], default_return=None
        )
    ...
    def log_iteration(self, recorded_values: ValueDict, iteration: int) -> None:
        ...
        """
        log_plots = (
            self.wandb_plot_log_frequency > 0
            and iteration % self.wandb_plot_log_frequency == 0
            and (iteration > 0 or self.plot_first_iteration)
        )
        log_video = (
            self.wandb_plot_log_frequency > 0
            and iteration % self.video_frequency == 0
            and (iteration > 0 or self.plot_first_iteration)
        )
        ...
        if log_plots:
            ...
            if keys.FIGURES in recorded_values:
                ...
                if isinstance(figures, list):  # multiple figures in a list
                    wandb_log_dict[keys.FIGURES] = {
                        f"{position}_{keys.FIGURES}": figure for position, figure in enumerate(figures)
                    }
                else:  # single figure
        ...
        if log_video and keys.VIDEO_ARRAYS in recorded_values:
            video_arrays = recorded_values.get(keys.VIDEO_ARRAYS)
            if isinstance(video_arrays, list):  # multiple figures in a list
                wandb_log_dict[keys.VIDEO_ARRAYS] = {
                    f"{position}_{keys.VIDEO_ARRAYS}": wandb.Video(video_array, format="mp4", fps=16)
    ...
    def finalize(self, final_values: ValueDict) -> None:
        ...
        if keys.SCALARS in final_values:
            self.wandb_logger.log({keys.SCALARS: final_values.get(keys.SCALARS)})
