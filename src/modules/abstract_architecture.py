...
from util.save_and_load.save_and_load_keys import (
    ARCHITECTURE,
    NETWORK_KWARGS_FILE,
    OPTIMIZERS,
    SCHEDULER,
...
class AbstractArchitecture(nn.Module, abc.ABC):
    ...
    def _initialize_optimizer_and_scheduler(self, training_config: ConfigDict) -> None:
        ...
        Args:
            ...
            Should contain the following keys:
                {optimizer - The optimizer to use. Should be a string that can be passed to get_optimizer,
                 ...
                 weight_decay - The weight decay/l2 norm to use for regularization,
        ...
        optimizer = get_optimizer(training_config.get("optimizer"))
        ...
        if lr_scheduling_rate is not None and lr_scheduling_rate < 1:
            self._learning_rate_scheduler = optim.lr_scheduler.ExponentialLR(
                optimizer=self._optimizer, gamma=lr_scheduling_rate
            )
        else:
    ...
    def save(
        ...
        if save_kwargs:
            save_dict(
                ...
                file_name=NETWORK_KWARGS_FILE,
                dict_to_save=self.kwargs,
                save_type="npz",
            )
    ...
    @gpu_device.setter
    def gpu_device(self, device: str):
    ...
    def load_from_path(
            ...
            network_kwargs: Optional[Union[ConfigDict, str]] = None,
        ...
        """
        from util.save_and_load.save_and_load_utility import (
            load_dict,
            undo_numpy_conversions,
        )
        ...
        assert issubclass(architecture_type, AbstractArchitecture), (
            f"Must inherit from AbstractArchitecture base class, " f"given {architecture_type} instead"
        )
        ...
        architecture: AbstractArchitecture = architecture_type(
            **network_kwargs,  # network params like latent dim etc.
        ...
        if device is not None:
            ...
            architecture.gpu_device = device
        return architecture
