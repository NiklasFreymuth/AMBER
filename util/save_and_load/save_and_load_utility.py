...
def undo_numpy_conversions(dictionary: dict) -> dict:
    ...
    for converting_type in [float, int, dict]:
        converted_dictionary = {
            k: v.item()
            if isinstance(v, np.ndarray) and (v.ndim == 0 or len(v) == 1) and isinstance(v.item(), converting_type)
            else v
            for k, v in converted_dictionary.items()
        }

    none_converted_dict = {
        k: None if isinstance(v, np.ndarray) and (v.ndim == 0 or len(v) == 1) and v == None else v
        for k, v in converted_dictionary.items()
    }
    tuple_converted_dict = {
        k: (v,) if isinstance(v, np.ndarray) and (v.ndim == 0 or len(v) == 1) and isinstance(v.item(), int) else v
